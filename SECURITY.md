...
The frontend kernel is the `smart-admin-web` npm package, released under the same number as the NuGet packages, so a security fix there arrives as a patch release you pick up by bumping the version. See the [upgrade guide](https://simpleone-x.github.io/SmartAdmin/guide/upgrade). The thin app template in `web/template` holds no kernel code: you copy it once and it belongs to your application from then on.
