 ...
 *
 * 前置(由 `playwright.config.ts` 立起):
 * - 后端是本次 run 自己起的 MinimalHost + 新建的临时 SQLite 库,带默认种子(5 个 scope_* 用户/角色,密码 123456)
 * - 种子角色初始状态:未授权任何菜单(仅有用户↔角色 + 数据范围关联)
 *
 * 测试会修改数据库(给角色授权菜单/用户),库每次 run 新建,不影响开发库。
 */
...
test.describe('RBAC 权限', () => {
  ...
  test('未授权菜单的用户 → 连应用都进不去,停在门户空态', async ({ page }) => {
    ...
    //
    // 不断言「侧边菜单 <= 1 项」,那是一句**恒真**的话:零授权用户在
    ...
    // 数出来永远是 0,菜单过滤坏成什么样它都绿。这里断言它实际发生的事。
    ...
    // 不另外断言 `.card` 为 0:空态与卡片网格在 `packages/admin/src/views/module/index.vue` 里是
  ...
  test('授权用户 → 添加用户 → 保存 → 回显 → 还原', async ({ page }) => {
    ...
    // .first() 不能删:本用例保存两次,而 Naive 的 toast 存活约 3 秒,机器快的时候两条
    // 「已保存」会同屏,裸 .n-message 命中 2 个元素直接 strict mode violation。
    await expect(page.locator('.n-message').first()).toBeVisible({ timeout: 5_000 })
