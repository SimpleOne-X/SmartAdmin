...
// 路由懒加载失败大多是发版后旧 chunk 404 —— 自动重载一次拿新 index.html(每标签页仅一次)。
router.onError(err => {
  loadingBar.error()
...
router.beforeEach(async to => {
  ...
  // 默认模式令牌已 localStorage 水合,ensureAccessToken 立即 true。
  ...
  // 强制改密守卫:管理员建号/重置后首登,未改密前只允许停留在改密页,阻断其它一切导航
  ...
  if (!auth.routesReady) {
    try {
      ...
      // 目标是 '/' 时不能 return to.fullPath——那是重定向到自身,'/' 没有静态 redirect,会被判成无限重定向。
      // 此刻菜单树已就绪,直接给出首页。
