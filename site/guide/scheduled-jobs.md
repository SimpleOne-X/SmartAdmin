...
Write a class, implement one method, and the admin UI can drive it on a cron. The scheduler runs inside your API process as part of `AddSmartAdmin()` — no package to install, nothing to configure, no extra process.
...
**The property bag is the only way in.** It's a string dictionary stored on the job row and handed to the handler through `context.Properties`. A property bag plus a twenty-line `IJob` beats a framework guessing your parameters.

SQL jobs are gated by `SmartAdmin:Jobs:Sql:Enabled`, `false` by default. Turning it on admits one thing: **whoever can edit a job now has DBA rights**.
...
**Jobs keep running with the API down.** An in-process scheduler cannot outlive its process, so this half needs a second one. Copy `backend/samples/WorkerHost`:

```csharp
var builder = Host.CreateApplicationBuilder(args);
// Must come before AddSmartAdminWorker: sharing a cache between the Worker and the API is a
// precondition for a multi-process deployment (forced logout, permission cache, rate-limit counts all ride on it)
...
Miss the `AddSmartAdminRedisCache` line and the kernel silently falls back to an in-process cache — the Worker and the API each count on their own, right back to the "no shared cache across replicas" problem; get the order backwards (after `AddSmartAdminWorker`) and it throws outright at wiring time instead.

A worker has three configuration rules: `SmartAdmin:Id:WorkerId` must be set explicitly and differ from every other process (it refuses to start otherwise), table creation and seeding must be off (the API owns the schema), and its timezone must match the API's.
...
**HTTP job targets go through a fence.** Blocked by default: the cloud metadata range (`169.254.0.0/16`, `fd00:ec2::/32`, and the IPv6 twin `fe80::/10`) and loopback (`127.0.0.0/8`, `::1/128`); private networks are not — reaching internal services is the point of an HTTP job (RFC1918 and ULA `fc00::/7` stay allowed). To target a local port, explicitly drop the loopback range from `SmartAdmin:Jobs:Http:BlockedCidrs`; to tighten instead, use the `SmartAdmin:Jobs:Http:AllowedHosts` allowlist. The fence checks at save time and at execution time, and re-checks the resolved IP when the connection is made, which is what catches a name that resolves publicly when saved and privately when run.
...
The whole module can be dropped: add `"Job"` to `SmartAdmin:Api:DisabledModules` and the controller's routes aren't even registered. To keep a replica out of scheduling while it still serves job queries and edits, set `SmartAdmin:Jobs:SchedulerEnabled=false`.
