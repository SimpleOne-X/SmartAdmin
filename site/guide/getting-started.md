...
::: tip Prerequisites
- .NET 10 SDK
- Node.js 22.12+, if you also want to run the frontend
:::
...
To start backend and frontend together locally, use `dev-start.bat` at the repo root — it brings up MinimalHost and the frontend Vite in two separate windows (running `npm install` in `web/` before Vite starts); `dev-stop.bat` stops them.
...
Then call `AddSmartAdmin` and `MapSmartAdmin` in `Program.cs`:
...
`web/` in the repository holds the source of the frontend package `smart-admin-web` plus a thin shell template, Vue 3 with Naive UI. Starting dev there runs the template, on `5173`:
...
::: tip Using it as your own project's frontend
...
Upgrading the kernel only changes the `smart-admin-web` version; the steps are in [Upgrading to a New Release](/guide/upgrade). Which files the template holds, and how your pages and text plug into the kernel, is covered in [The Frontend Template](/guide/frontend-templates).
:::

## Swap out the default database

Zero-config defaults to SQLite (`Data Source=./data/SmartAdmin.db`, relative to the ContentRoot). Switching to a real database takes no code changes — the `SmartAdmin:Database` section decides it, and you change just two things, `DbType` and `ConnectionString` (`Sqlite` / `MySql` / `SqlServer` / `PostgreSQL` are all supported):
