...
```csharp
// backend/src/SmartAdmin.SqlSugar/SqlSugarSetup.cs
...
A consumer must register **before** `AddSmartAdmin()` to win. Register after, and the built-in service has already claimed the slot — `TryAdd` silently skips the consumer's registration. No error is raised; the replacement just silently doesn't take effect.
:::

The optional `SmartAdmin.Caching.Redis` package is the canonical example: it `TryAdd`s the Redis implementation of `ICacheProvider` before `AddSmartAdmin()` runs, taking precedence over the kernel's default in-process `MemoryCacheProvider`.

```csharp
// backend/samples/MinimalHost/Program.cs
builder.Services.AddSmartAdminRedisCache(builder.Configuration); // register first, wins the TryAdd
...
```csharp
// backend/tests/SmartAdmin.Tests/ReplaceabilityTests.cs
...
## `ReplaceabilityTests` locks these down as a contract

`backend/tests/SmartAdmin.Tests/ReplaceabilityTests.cs` is the regression lock for the replaceability mechanism, and the extension-point list is registered in `ReplaceabilityContract.cs` alongside it. The test names are deliberately fixed by design, verifying the three constraints above as a contract, not as ordinary tests. Below are a few representative cases — the full list is whatever those two files say:
...
```csharp
// backend/src/SmartAdmin.Services/Module/ModuleService.cs
...
// 2. Register before AddSmartAdmin() (order is what matters)
