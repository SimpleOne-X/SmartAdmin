#!/usr/bin/env pwsh
# SmartAdmin template smoke test: pack core -> local feed -> pack template -> install -> scaffold -> build.
# Proves the generated smart-app compiles against the real kernel packages.
...
param(
    ...
    # A release run passes the real tag version instead; same proof, against the actual artifacts.
    [string]$Version = '9.9.9-smoke'
)
$ErrorActionPreference = 'Stop'

$repo = (Resolve-Path "$PSScriptRoot/..").Path
$work = Join-Path ([System.IO.Path]::GetTempPath()) ("smart-tmpl-" + [guid]::NewGuid().ToString('N').Substring(0, 8))
...
# (the reference app itself). A non-hyphenated scaffold name cannot see that class of bug at all; it shipped
...
# Without this the test is only as honest as the dev box: a SmartAdmin left in the real global cache by an
...
try {
    ...
    try {
        ...
        # 'dotnet new' reports its own exit code 0 even when the automatic post-action restore it
        # triggers fails (the failure prints as a warning, not a nonzero exit) -- so the same transient
        # file lock that hits an explicit restore can hit this one silently. Check the actual evidence
        # (project.assets.json) instead of trusting the exit code alone.
        Invoke-WithRetry -Msg 'dotnet new + automatic restore' -Action {
            dotnet new smart-app -n $name -o $out
            if ($LASTEXITCODE -eq 0 -and -not (Test-Path $assets)) {
                Write-Host "-- dotnet new reported success but $assets is missing (post-action restore failed silently)"
        ...
        if ($csproj -notmatch [regex]::Escape("Version=`"$ver`"")) {
            throw "FAILED: generated project does not reference the packed version $ver. The template default was not stamped at pack time. csproj says: $(($csproj | Select-String 'PackageReference.*SmartAdmin' -AllMatches).Matches.Value -join ', ')"
        }
        # The scaffolded Tests project references SmartAdmin.Testing through the same stamp; a drift there is just as
        # invisible as one in the host csproj (a stale minimum floats up to whatever the feed has), so it gets the same assertion.
        $testsCsproj = Get-Content (Join-Path $out 'Tests/Tests.csproj') -Raw
        if ($testsCsproj -notmatch [regex]::Escape("Include=`"SmartAdmin.Testing`" Version=`"$ver`"")) {
            throw "FAILED: scaffolded Tests/Tests.csproj does not reference SmartAdmin.Testing $ver. Tests csproj says: $(($testsCsproj | Select-String 'PackageReference.*SmartAdmin' -AllMatches).Matches.Value -join ', ')"
        }

        # The generated Dockerfile is never built here (no docker in CI for this job), so a hyphenated
        # scaffold name alone would still not catch the content-substitution bug: it once wrote
        ...
        try {
            ...
            if (-not $healthy) {
                ...
                throw "FAILED: generated smart-app did not serve /health from a plain 'dotnet run'.`n$tail"
            }
        }
        finally {
        ...
        # The scaffolded Tests/ project is the consumer's regression net; ship it broken and every consumer
        ...
        $gitignore = Join-Path $out '.gitignore'
        if (-not (Test-Path $gitignore)) {
            throw 'FAILED: scaffolded project has no .gitignore -- "git add ." would commit data/dev-jwt.key (forgeable super-admin tokens).'
        }
        $gi = Get-Content $gitignore -Raw
        foreach ($p in @('data/', 'bin/', 'obj/', 'appsettings.Development.json')) {
            if ($gi -notmatch [regex]::Escape($p)) { throw "FAILED: scaffolded .gitignore does not cover '$p'." }
        }

        Write-Host "`n[OK] smoke passed: generated smart-app compiles and runs against the real kernel."
    }
    finally {
        dotnet new uninstall SmartAdmin.Templates 2>$null | Out-Null
    }
}
finally {
