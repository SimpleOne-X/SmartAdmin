...
> 本文件是术语表,只回答「这个词在本仓精确指什么」,不含实现细节。词条按 `docs/agents/domain.md` 的约定逐模块懒创建。
...
| 术语 | 定义 |
|---|---|
| 内核 | `backend/src/SmartAdmin.{Core,SqlSugar,Services,AspNetCore}` 四包 + 元包 `SmartAdmin`。 |
| 可选包 | 可选独立 NuGet 包(`SmartAdmin.Excel`、`SmartAdmin.Caching.Redis`、`SmartAdmin.Auth.*`),装了才有。 |
| 消费者 | 安装这些包搭自己后台的开发者;其程序集经 `options.ApplicationAssemblies` 挂入。 |
| 可替换性契约 | `ReplaceabilityTests` 锁死的可替换性保证(内置服务 TryAdd 注册、前置注册即胜出、方法 `virtual` 可继承覆写等),扩展点清单见 `ReplaceabilityContract.cs`。 |
...
| 术语 | 定义 |
|---|---|
| 定级对象 | 实际承载业务、数据和用户的已部署信息系统。等保级别由业务与环境影响决定;**SmartAdmin 内核本身不是定级对象**，不得宣称内核「已通过等保」。 |
...
| 双提交 CSRF | Cookie 会话下的防护：可读 csrf Cookie + 写请求 `X-Smart-CSRF` 头，服务端常量时间比对。 |
