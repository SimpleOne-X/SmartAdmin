...
/// <summary>
/// 用户表:账号口令、个人资料、组织归属(机构/职位/主管)与登录安全状态。
/// 新增列由 CodeFirst 自动补上,演进列必须可空(见 <see cref="ForceTotp"/>)。
/// </summary>
...
public class SysUser : BaseEntity
{
    ...
    /// <summary>停用后无法登录、已有会话由权限过滤器拦截</summary>
    [SugarColumn(ColumnDescription = "是否启用")]
    public bool Enabled { get; set; } = true;

    /// <summary>
    /// 超级管理员标志:授权管道第一步就直接放行,不受 RBAC 约束。
    ...
    /// null = 没有改密时间记录的存量用户:首次登录时回填为当时时间,过期窗口从那一刻起算,
    /// 开启过期策略当天存量用户不会被一起判过期。
    /// </summary>
    ...
    /// 存量行在库里是 NULL,SqlSugar 物化为 false;公共属性保持 bool,CLR 类型不变。
