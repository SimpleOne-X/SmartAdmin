...
/// <summary>
/// 异常日志——由 <c>ExceptionLogFilter</c> 在未捕获异常(非 <c>AdminException</c> 业务异常)冒泡时自动写入。
/// 记录"哪个接口、什么方法、追踪号、异常类型、消息、堆栈、谁触发的、从哪来"。
/// <para>与 <see cref="SysOpLog"/>/<see cref="SysLoginLog"/> 一样只增不改、无软删语义,"清空"走硬删。
/// 只落异常本身(类型/消息/堆栈),<b>绝不落请求响应体</b>——响应体可能含明文口令/令牌。
...
public class SysExceptionLog : TenantEntity
{
    ...
    /// <summary>请求追踪号(有 W3C 追踪上下文时取其 TraceId,否则回退 <c>HttpContext.TraceIdentifier</c>);串起同一请求的诊断日志与本条异常记录</summary>
