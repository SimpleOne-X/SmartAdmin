...
/// <summary>
/// 系统通知表(消息中心,轮询模型)——管理员发布的消息,可发给全体 / 指定角色 / 指定用户。
...
public class SysNotice : TenantEntity
{
    ...
    /// <summary>
    /// 可选的结构化动作列表(JSON 数组,元素形状见 <see cref="NoticeAction"/>),前端渲染成正文下的按钮。
    /// 服务端只存不解释:不执行动作、不校验目标端点是否存在,权限由目标端点自己判。
    /// 可空:旧行 NULL,即"没有动作",不需要迁移。
    /// </summary>
    [SugarColumn(Length = 1000, IsNullable = true, ColumnDescription = "动作列表(JSON)")]
