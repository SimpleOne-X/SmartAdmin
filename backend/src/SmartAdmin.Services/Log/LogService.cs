...
public class LogService(
    ...
    IDataScopeGuard? scopeGuard = null,
    // 导出行数上限;可选尾参,未注入时用默认 50000
    AdminExcelOptions? excel = null,
    // 操作日志异步写手 + 其配置;可选尾参,未注册时退回同步落库(纯 Services 宿主)
    AdminLoggingOptions? logging = null,
    OperationLogWriter? opLogWriter = null,
    // 统一时间源;可选尾参,未注入时退回系统时钟(与审计 AOP 同口径)
    TimeProvider? time = null) : ILogService
{
    private AdminExcelOptions Excel => excel ?? new AdminExcelOptions();

    /// <summary>
    /// 范围内的用户 Id;不受限或未注入守卫时为 null(不加条件)。
    /// <para>日志是审计面:非超管只该看见自己范围内的人做过什么,否则一个子管理员就能读到全公司的操作日志
    /// (含入参 JSON),因此操作/登录/异常三张日志表的查询都按这层数据范围收敛。</para>
    /// </summary>
    protected virtual Task<List<long>?> ScopedOperatorIdsAsync() =>
        scopeGuard is null ? Task.FromResult<List<long>?>(null) : scopeGuard.ResolveScopedUserIdsAsync();

    /// <inheritdoc />
    public virtual async Task RecordOperationAsync(OperationLogEntry entry)
    {
        var row = BuildOperationRow(entry);

        // 异步:入队即返回,批量插入在后台跑(默认)。同步:老路,落库失败只告警。
        if (opLogWriter is not null && logging?.OpLog.Sync != true)
        {
            opLogWriter.Enqueue(row);
            return;
        }

        try
        {
            await opLogs.InsertAsync(row);
        }
        catch (Exception ex)
        {
            // 日志写入失败只告警,绝不冒泡破坏已完成的业务请求
    ...
    {
        var page = await (await ScopedOpListQueryAsync(input)).ToPagedListAsync(input.Current, input.Size);
    ...
    public virtual async Task<IReadOnlyList<SysOpLog>> ExportOpLogsAsync(OpLogPageInput input)
    {
        ...
        var items = await (await ScopedOpListQueryAsync(input)).Take(max + 1).ToListAsync();
    ...
    /// <summary>
    /// 操作日志骨架 + 数据范围:匿名行(OperatorId 为 null)只有不受限者可见——认不出是谁做的,
    /// 就无从判断它属于哪个机构。
    /// </summary>
    protected virtual async Task<ISugarQueryable<SysOpLog>> ScopedOpListQueryAsync(OpLogPageInput input)
    {
        var ids = await ScopedOperatorIdsAsync();
        return BuildOpListQuery(input)
            .WhereIF(ids != null, x => x.OperatorId != null && ids!.Contains(x.OperatorId.Value));
    }
    ...
    {
        var scopedIds = await ScopedOperatorIdsAsync();
        var page = await loginLogs.AsQueryable()
            .WhereIF(scopedIds != null, x => x.UserId != null && scopedIds!.Contains(x.UserId.Value))
    ...
    public virtual async Task<PagedList<SysExceptionLog>> PageExceptionAsync(ExceptionLogPageInput input)
    {
        ...
        var page = await exceptionLogs.AsQueryable()
            .WhereIF(scopedIds != null, x => x.OperatorId != null && scopedIds!.Contains(x.OperatorId.Value))
            .WhereIF(!string.IsNullOrEmpty(input.Path), x => x.Path.Contains(input.Path!))
