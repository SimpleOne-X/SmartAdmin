    ...
    ILogger<JobExecutor> logger)
{
    ...
    /// <b>同步</b>登记,把同节点的窗口彻底闭死;跨节点残余窗口(主切换瞬间)仍在。
    /// </summary>
    ...
    /// <summary>
    /// 停机排水:停发新触发 → 宽限期内等在飞收尾 → 逾期(hardStop 触发)逐个 Cancel 再等闭合。
    /// </summary>
    ...
    protected virtual async Task RunFireAsync(SysJob job, DateTime scheduledTime, JobFireMode fireMode, long fireInstanceId)
    {
        try
        {
            ...
            for (var retryIndex = 0; retryIndex <= retryCount; retryIndex++)
            {
                ...
                if (outcome != JobRunStatus.Failed || !retryable) break;   // 成功收工;超时/取消/处理器缺失不重试
    ...
    private static (JobRunStatus Status, string Note) Interpret(SysJob job, CancellationTokenSource? timeoutCts, bool ignoredToken)
    {
        ...
        if (ignoredToken) note += ";处理器未响应取消令牌,实际跑到自然结束(任务实现必须传递取消令牌)";
        return (timedOut ? JobRunStatus.Timeout : JobRunStatus.Cancelled, note);
    }

    /// <summary>跨节点终止:每 KillPollSeconds 按主键读<b>自己那行</b>的 KillRequested 旗标。</summary>
    ...
    {
        // 每次触发都要经这里收尾,埋在这一处就够,不必在每条 return 路径上各记一次
        SmartAdminDiagnostics.JobRuns.Add(1, new KeyValuePair<string, object?>("result", status.ToString()));
    ...
    /// <summary>
    /// 一次触发收尾:计数、OneShot 完结、连败 Panic 与告警。
    ...
    protected virtual async Task SendPanicAlertAsync(SysJob job, int consecutive, string? lastError)
    {
        ...
        // 站内信与邮件彼此独立:接收目标校验失败或精简宿主缺用户表时,不能拖死邮件通道。
