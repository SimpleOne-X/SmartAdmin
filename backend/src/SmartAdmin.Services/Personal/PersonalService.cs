...
public class PersonalService(
    ...
    // 统一时间源:可选尾参,DI 正常注入,消费者子类不传也能编译;LastPasswordChangeTime 这类持久化时间戳走注入时钟,便于测试固定时间
    TimeProvider? time = null,
    // 自助改密后吊销"其它"会话(保留当前会话,使本次响应正常返回);尾随可选,消费者子类省略也能编译
    ...
    IRepository<SysLoginLog>? loginLogs = null) : IPersonalService
{
    ...
    public virtual async Task ChangePasswordAsync(long userId, ChangePasswordInput input)
    {
        ...
        user.MustChangePassword = false;   // 自助改密成功即清除强制改密标志
