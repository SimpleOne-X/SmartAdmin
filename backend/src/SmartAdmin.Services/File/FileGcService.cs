...
/// <summary>
/// 磁盘回收后台任务。上传子系统有两处只涨不消的地方,都在这里收:
/// <list type="number">
    ...
    ICacheProvider? cache = null) : BackgroundService   // 可选参数:本类是 public 可继承,加必需参数即源码破坏性变更
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        if (!options.EnableGc)
        {
            logger.LogInformation("SmartAdmin 磁盘回收已关闭(Upload:EnableGc=false):软删文件与弃单分片将不会被回收。");
            return;
        }
        ...
        while (await timer.WaitForNextTickAsync(stoppingToken))
        {
            try
            {
                ...
                if (files > 0 || sessions > 0)
                    logger.LogInformation("SmartAdmin 磁盘回收:清理 {Files} 个软删文件、{Sessions} 个弃单分片会话。", files, sessions);
            }
            ...
            catch (Exception ex)
            {
                ...
                logger.LogWarning(ex, "SmartAdmin 磁盘回收:本轮失败,下个周期重试。");
            }
        }
    }
    ...
    {
        if (cache is null) return true;   // 没有缓存实现(消费者自建实例)→ 退回"人人都扫"(GC 幂等,见上)
    ...
    protected virtual async Task<int> ReclaimDeletedFilesAsync(CancellationToken cancellationToken)
    {
        ...
        // COALESCE/日期函数较劲。UpdateTime 是删除时刻(见 SqlSugarRepository.DeleteAsync);没有该戳的行
        // 回退到 CreateTime,否则它们永远收不掉。
        // 同样必须清 ITenantScoped:本服务是无 HttpContext 的后台单例(BackgroundService.ExecuteAsync 自开作用域),
        ...
        foreach (var file in due)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.LogWarning(ex, "SmartAdmin 磁盘回收:跳过文件 Id={Id},StoragePath={Path}", file.Id, file.StoragePath);
            }
        }
        return removed;
    }
