...
public class UserService(
    ...
    // 可选参数:默认 DI 注入,消费者子类省略也能编译。externalBindings 保留为构造参数(消费者子类可能依赖它);
    // 软删不清绑定,真正清理由回收站 Purge 经 DI 取 ISysUserExternalService 完成。
    IPasswordHistoryService? passwordHistory = null,
#pragma warning disable CS9113
    ISysUserExternalService? externalBindings = null,
#pragma warning restore CS9113
    // 可选尾参 TimeProvider:DI 正常注入,消费者子类不传也能编译;LastPasswordChangeTime 等持久化时间戳走注入时钟
    ...
    // 数据范围(非超管只能管理范围内机构的用户)+ 当前用户上下文
    IDataScopeContext? dataScope = null,
    ICurrentUser? currentUser = null,
    // 系统表上的范围收口;未注入时按当前上下文现造一个,老子类不传也能跑
    IDataScopeGuard? scopeGuard = null) : IUserService
{
    ...
    /// <inheritdoc />
    // ponytail: .Contains 生成参数化 LIKE(无注入风险),但未转义 LIKE 元字符 % _——
    ...
    {
        // 范围条件收口到 IDataScopeGuard:系统表不受全局过滤器管,由它统一收口而不是让各服务各写一份
        return Guard.ScopeUsers(users.AsQueryable())
    ...
    public virtual async Task DeleteAsync(long id)
    {
        ...
        await InTransactionAsync(async () =>
        {
            // 软删不清角色/外部绑定:关联保留,恢复即可用;真正清理在回收站 Purge
    ...
    public virtual async Task DeleteBatchAsync(IReadOnlyCollection<long> ids)
    {
        ...
        // 整批包一个事务:任一步失败全回滚。软删不清角色/外部绑定(关联保留,恢复即可用;真正清理在 Purge)。
        await InTransactionAsync(async () =>
        {
    ...
    public virtual async Task<string> ResetPasswordAsync(long id, string? newPassword)
    {
        ...
        user.MustChangePassword = true;   // 管理员重置:强制用户下次登录后改密
    ...
    /// <summary>
    /// 校验给定机构在调用者的数据范围内。超管与不受限范围一律放行;
    /// <paramref name="orgId"/> 为 null 也放行(用户未分配机构)。
    /// </summary>
    protected virtual void ValidateOrgInScope(long? orgId)
    {
        Guard.EnsureOrgInScope(orgId);
    }
    ...
    {
        await users.Db.RunInTransactionAsync(work);
    }
}
