...
/// </summary>
public class DefaultJobSeed : ISeedData<SysJob>
{
    ...
    /// </summary>
    public virtual bool SyncOnUpgrade => false;

    /// <inheritdoc />
    public virtual IEnumerable<SysJob> HasData() =>
    [
        new SysJob
        {
            Id = 1,
            ...
            // NextRunTime 留空:调度器重载时对 Ready 且 NextRunTime 为空的行按触发配置补算,
            // 种子在编写期没有时钟,不硬编码时刻。
            IsSystem = true,
            AlertByNotice = true,
