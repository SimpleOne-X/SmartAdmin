...
/// <summary>
/// 超级管理员种子:零配置启动时自动创建超管账号。
/// <para>密码策略:配置了 <c>SmartAdmin:Seed:AdminPassword</c> 用配置值;
    ...
    ILogger<SuperAdminSeed> logger) : ISeedData<SysUser>
{
    ...
    public virtual IEnumerable<SysUser> HasData()
    {
        ...
        // 只画左边框:中文在控制台里占两格、在网页里宽度不定,右边框对不齐反而像坏了
        if (generated)
            logger.LogWarning("""
            ...
            logger.LogInformation("SmartAdmin: 已按配置创建超级管理员 {Account}", options.Seed.AdminAccount);
