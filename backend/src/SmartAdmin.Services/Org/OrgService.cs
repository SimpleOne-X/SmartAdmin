...
public class OrgService(
    ...
    IRbacService rbac,
    // 可选参数放在末尾:子类覆写时不必被迫接住全部依赖
    IDataScopeContext? dataScope = null,
    ICurrentUser? currentUser = null,
    IRepository<SysUser>? userRepo = null,
    // 系统表上的范围收口;未注入时按当前上下文现造一个,老子类不传也能跑
    IDataScopeGuard? scopeGuard = null) : IOrgService
{
    /// <summary>数据范围守卫(机构可见性与越权判定收口在它)。</summary>
    protected IDataScopeGuard Guard =>
        scopeGuard ?? new DataScopeGuard(userRepo ?? throw new InvalidOperationException(
            "OrgService 需要 IDataScopeGuard 或 IRepository<SysUser> 之一才能判定数据范围。"), currentUser, dataScope);
    ...
    public virtual async Task<IReadOnlyList<SysOrg>> ListAsync()
    {
        ...
        // 非超管只看范围内机构,外加它们的祖先链(树形展示要的结构节点)
    ...
    public virtual async Task<long> AddAsync(OrgInput input)
    {
        ...
        // 编码留空则自动生成 10 位随机字母数字串(沿用参照实现的做法)
        var code = input.Code;
    ...
    public virtual async Task UpdateAsync(long id, OrgInput input)
    {
        ...
        // 父指向自己是非法父级,用专用码 OrgInvalidParent(42008)——父机构本身存在,只是指向不合法,OrgNotFound 语义不符
        ...
        // 编码留空 = 不改(与新增时"留空自动生成"呼应,空值不能写进唯一列);改编码时排除自身查重(纳入软删行)
    ...
    public virtual async Task<long> CopyAsync(long id, string? newName = null)
    {
        ...
        await orgs.Db.RunInTransactionAsync(async () =>
        {
            while (queue.Count > 0)
            {
    ...
    /// <paramref name="orgId"/> 为 null 表示根级(新增顶级机构),视为在范围内。
    /// </summary>
    protected virtual void ValidateOrgInScope(long? orgId)
    {
        // null = 根级(新增顶级机构),沿用既有语义视为在范围内
