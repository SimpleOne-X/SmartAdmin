    ...
    IPermissionProvider? permissions = null) : IMfaEnrollmentService
{
    ...
    /// <summary>运行时总闸(SysConfig ∨ Options);无 policy 时回退 Options 同步判定。</summary>
    ...
    public virtual async Task<TotpBindCompleteOutput> CompleteBindAsync(TotpBindCompleteInput input)
    {
        ...
        {
            logger.LogWarning(ex, "TOTP 绑定挑战的种子解密失败(userId={UserId}),按绑定无效返回", payload!.UserId);
            throw new AdminException(ErrorCode.MfaBindInvalid);
        }
    ...
    /// <summary>
    /// 清空用户的 MFA 绑定状态(TOTP 密钥、绑定时间)与全部恢复码;
    /// <paramref name="revokeSessions"/> 为 true 时一并吊销其全部会话。
    /// </summary>
    ...
    /// <summary>
    /// 整体重置某用户的恢复码:先删旧的,再生成 <see cref="RecoveryCodeCount"/> 个新码并落库(存哈希),
    /// 返回明文供本次一次性展示。
    /// </summary>
    ...
    /// <summary>生成一个人类可读的恢复码:三组 4 位字符,短横分隔,字符集排除易混淆字符。</summary>
    ...
    /// <summary>恢复码规范化:去空白与短横并转大写,用于哈希前对齐用户粘贴/手输的各种格式。</summary>
