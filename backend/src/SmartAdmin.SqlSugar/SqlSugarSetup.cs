...
/// <summary>
/// 数据层装配:SqlSugar 单例 + 泛型仓储 + 种子机制 + 首启初始化。
/// 注册一律 TryAdd——用户在 AddSmartAdmin() 之前注册同接口实现即可整体替换。
/// <para>多库:主库 ConfigId 固定 <see cref="MainConfigId"/>;副库经
/// <c>additionalDatabases</c> / <c>SmartAdmin:AdditionalDatabases</c> 挂入同一
...
public static class SqlSugarSetup
{
    ...
    public const string MainConfigId = "SmartAdmin";
    ...
    /// </exception>
    public static IServiceCollection AddSmartAdminSqlSugar(
        ...
        IEnumerable<AdminDatabaseConnectionOptions>? additionalDatabases = null)
    {
        ...
        // WorkerId 取 AdminIdOptions 的有效值:显式配置的 SmartAdmin:Id:WorkerId,或组合根用文件锁抢到的本机槽位(WorkerIdLease,
        // 首次解析时回写)。跨机器/跨容器仍须每实例配不同值,否则同毫秒撞号。裸容器无 AdminIdOptions 时回落 0。
        ...
        // ── 数据范围环境载体:授权管道写入、全局过滤器读取;AsyncLocal 单例 ──
        ...
        var sources = new SmartEntitySources();
        ...
        services.TryAddSingleton<ISqlSugarClient>(sp =>
        {
            ...
            // SQLite 相对路径规整为相对 ContentRoot 的绝对路径:否则从非项目目录/服务托管启动时,
            ...
            var sqlLog = (sp.GetService<ILoggerFactory>() ?? NullLoggerFactory.Instance).CreateLogger("SmartAdmin.Sql");
            // 控制台 SQL 日志:主库副库同一个开关(排查时要看的是"这次请求打了哪些库",按库分别开没有用),
            // 块头里的 ConfigId 区分是谁。颜色只判一次:每条语句都去探一遍终端是白费。
            var sqlLogOptions = db.SqlLog;
            var sqlLogColor = sqlLogOptions.Enabled && SqlLogFormatter.ShouldColor(sqlLogOptions.Color);

            if (additionals.Count > 0)
            {
                // 引导日志:副库实体勿进 ApplicationAssemblies,否则主库 CodeFirst 会误建表。
                sqlLog.LogInformation(
                    "SmartAdmin: 已挂 {Count} 个副库 ConfigId=[{Ids}]。访问请用 db.AsTenant().GetConnection(configId);" +
            ...
            void AttachHooks(ISqlSugarClient client, string configId, HookPolicy policy)
            {
                ...
                {
                    // 全局软删过滤器:实现 ISoftDelete 的实体,查询自动排除已删行
                    client.QueryFilter.AddTableFilter<ISoftDelete>(e => e.IsDelete == false);
                }

                if (policy.ApplyDataScopeFilter)
                {
                    // 全局数据范围过滤器(招牌能力):对 IOrgScoped 实体(DataEntity 及其子类)按当前请求生效范围过滤。
                    ...
                    client.Aop.DataExecuting = (_, info) =>
                    {
                        switch (info.OperationType)
                        {
                            case DataFilterType.InsertByObject:
                                ...
                                // 按 PrimaryId 匹配(而非 BaseEntity):BaseEntity : PrimaryId,故审计实体一并覆盖,
                                // 且明细/子表(仅继承 PrimaryId,无审计字段)插入时同样自动获得雪花 Id。
                                ...
                                // CreateOrgId 未指定(实现 IOrgScoped 的实体有此列:DataEntity / OrgAuditEntity)→ 填当前用户归属机构(数据范围锚点);
                ...
                if (threshold > 0 || sqlLogOptions.Enabled)
                {
                    client.Aop.OnLogExecuted = (sql, pars) =>
                    {
                        ...
                        // 统计始终累加(只要开了控制台日志):汇总行要数出 N+1,而 N+1 的每一条都不慢
                        var index = sqlLogOptions.Enabled ? SqlLogScope.Current?.Record(millis) : null;

                        if (sqlLogOptions.Enabled && millis >= sqlLogOptions.MinMillis)
                        {
                            sqlLog.Log(slow ? LogLevel.Warning : LogLevel.Information,
                                SqlLogFormatter.BLOCK_TEMPLATE,
                                SqlLogFormatter.BlockArgs(
                                    index, sql, pars, millis, configId,
                                    sqlLogOptions.MaxSqlChars, sqlLogColor, threshold));
                            return;   // 已经打过了,别再走下面那条慢 SQL 单行
                        }

                        if (slow)
                        {
                            sqlLog.LogWarning("慢 SQL[{ConfigId}]({Elapsed}ms ≥ {Threshold}ms): {Sql} | 参数: {Parameters}",
                                configId, (long)millis, threshold, sql, FormatSqlParameters(pars));
                        }
                    };
                }
            }

            // SqlSugar 5.1.4.198 版本实测结论:
            ...
            // 查表+挂钩必须在同一把锁内完成:TryGetValue→Attach→Add 非原子时,
            // 两线程可能对同一 client 双挂软删过滤器并互相覆盖 OnError/DataExecuting。
            // 新上下文新实例再挂一次,跨 SuppressFlow 仍生效。单库时 additionals 为空,没有副库要挂。
    ...
    /// <summary>
    /// 摘掉一个已注册的种子实现。
    /// <para>种子走 <c>TryAddEnumerable</c> 收集,消费者注册自己的子类不会顶掉内置那份——两份都会跑,同 Id 行按主键判存
    /// 谁先跑谁生效,结果取决于注册顺序。要用子类整体替换内置种子(如改内置菜单树),先摘掉内置实现:
    /// <c>services.RemoveSeed&lt;DefaultMenuSeed&gt;().TryAddEnumerable(ServiceDescriptor.Transient&lt;ISeedData, MyMenuSeed&gt;())</c>。</para>
    /// </summary>
    public static IServiceCollection RemoveSeed<TSeed>(this IServiceCollection services) where TSeed : ISeedData
    {
        for (var i = services.Count - 1; i >= 0; i--)
            if (services[i].ServiceType == typeof(ISeedData) && services[i].ImplementationType == typeof(TSeed))
                services.RemoveAt(i);
        return services;
    }
    ...
    internal static void ValidateAdditionalDatabases(IReadOnlyList<AdminDatabaseConnectionOptions> additionals)
    {
        ...
        // 保留名与唯一性按忽略大小写:避免 smartadmin / AUDIT+audit 绕过
        var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        for (var i = 0; i < additionals.Count; i++)
        {
            var item = additionals[i] ?? throw new InvalidOperationException(
                $"SmartAdmin:AdditionalDatabases[{i}] 为 null。");
            var id = item.ConfigId?.Trim() ?? "";
            if (id.Length == 0)
                throw new InvalidOperationException(
                    $"SmartAdmin:AdditionalDatabases[{i}].ConfigId 不能为空。");
            if (string.Equals(id, MainConfigId, StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException(
                    $"SmartAdmin:AdditionalDatabases[{i}].ConfigId 不能为保留值 \"{MainConfigId}\"(主库专用,大小写不敏感)。");
            if (!seen.Add(id))
                throw new InvalidOperationException(
                    $"SmartAdmin:AdditionalDatabases 存在重复 ConfigId \"{id}\"(大小写不敏感)。");
            // 回写 Trim 后的 Id,避免配置两侧空白导致 GetConnection 对不上(大小写保持用户写法,与 SqlSugar 一致)
            item.ConfigId = id;
            if (string.IsNullOrWhiteSpace(item.ConnectionString))
                throw new InvalidOperationException(
                    $"SmartAdmin:AdditionalDatabases[{i}](ConfigId={id}) ConnectionString 不能为空。");
            if (!Enum.TryParse<DbType>(item.DbType, ignoreCase: true, out _))
                throw new InvalidOperationException(
                    $"SmartAdmin:AdditionalDatabases[{i}](ConfigId={id}) 未知 DbType \"{item.DbType}\"。" +
    ...
    /// <summary>
    /// SQL 参数渲染成 <c>@p0=1, @name=张三</c>;<b>名字像口令/密钥/令牌的参数只打 <c>***</c></b>。
    /// <para>没有参数值,同一条失败的 SQL 就复现不出来 —— 只有语句是不够的。但值里可能有口令哈希、
    /// TOTP 恢复码、消费者业务表的敏感列,而失败 SQL 的日志是关不掉的,慢 SQL 阈值调低时更是每条都打。
    /// 名单与操作日志入参脱敏共用 <see cref="SensitiveKeys"/>。</para>
    /// </summary>
    private static string FormatSqlParameters(object? parameters) => parameters switch
    {
        SugarParameter[] ps when ps.Length > 0 => string.Join(", ",
            ps.Select(p => $"{p.ParameterName}={(SensitiveKeys.IsSensitive(p.ParameterName) ? "***" : p.Value)}")),
        _ => "(无)",
    };
