...
/// <summary>
/// 外部登录授权态:授权阶段生成、存缓存(<see cref="Core.CacheKeys.OAuthState"/>),回调阶段一次性取出。
...
public record ExternalProviderAdminItem
{
    ...
    /// <summary>对应 <c>sys.externalauth.{code}.linkByAccount</c>(未绑定的外部身份按账号名关联同名本地账号);缺省 false。</summary>
    public bool LinkByAccount { get; init; }
}

/// <summary>个人中心"账号绑定"列表项(只回展示所需,不回 Subject 等原始标识)。</summary>
public record ExternalBindingItem
{
    /// <summary>provider 码(如 <c>github</c>/<c>wechat</c>)。</summary>
...
public class ExternalPendingLinkPayload
{
    ...
    /// <summary>该身份在外部 provider 侧的唯一标识(OAuth/OIDC 的 subject,不是本系统的用户 Id)。</summary>
...
{
    /// <summary>待绑定缓存的键(即 <see cref="ExternalPendingLinkPayload"/> 的存储键),不透传原始身份信息。</summary>
