...
/// <summary>
/// 外部登录 / SSO 端点。<c>[Module("ExternalAuth")]</c> 可整段禁用。机密客户端:client secret 全程服务端持有,
    ...
    AuthCookieService cookies) : ControllerBase
{
    ...
    public async Task<Result<IReadOnlyList<ExternalProviderAdminItem>>> ProvidersAll(CancellationToken cancellationToken)
    {
        ...
        foreach (var p in await providerRegistry.ListAsync(cancellationToken))
        {
            items.Add(new ExternalProviderAdminItem
            {
                ...
                LinkByAccount = await externalBindings.IsLinkByAccountAsync(p.Code),
            });
        }
    ...
    /// <summary>一次性票据换令牌(登录回调后前端调用;票据无效/过期/已用抛 40014)。Cookie 会话模式同步写 Cookie/CSRF。</summary>
    [HttpPost("exchange")]
    [AllowAnonymous]
    ...
    {
        // state 只用字母数字:企业微信 / 微信的 OAuth 只接受 a-zA-Z0-9、不超过 128 字节,base64url 的 - 和 _ 不在其内
        ...
        return await provider.BuildAuthorizeUrlAsync(
            new ExternalAuthorizeRequest(state, nonce, CodeChallenge(verifier), redirectUri)
            {
