...
/// <summary>
/// 定时任务配置(对应 <c>SmartAdmin:Jobs</c> 节)。
/// <para>
...
{
    /// <summary>本副本是否参与选主/调度,默认 true;false = 纯 API 副本(执行一次/查询/编辑照常)</summary>
    public bool SchedulerEnabled { get; set; } = true;

    /// <summary>节点名(集群内唯一);空 → <c>{MachineName}#{WorkerId}</c></summary>
    public string? NodeName { get; set; }

    /// <summary>心跳间隔(秒),默认 10:节点行 upsert + 主节点续租/备节点夺租的节拍;必须为正数(绑定时校验)</summary>
    public int HeartbeatSeconds { get; set; } = 10;

    /// <summary>选主租约时长(秒),默认 30;必须 > 2×<see cref="HeartbeatSeconds"/>(绑定时校验),容一次 GC 停顿或一次 DB 抖动</summary>
    public int LeaseSeconds { get; set; } = 30;

    /// <summary>任务缓存全量重载间隔(秒),默认 30——跨副本配置收敛上限(事件总线仅进程内,别的副本改任务靠它兜底);必须为正数</summary>
    public int ReloadSeconds { get; set; } = 30;

    /// <summary>到期时刻迟到超过此秒数才算「错过」,默认 60,走任务行上的 MisfireStrategy;以内正常触发;必须为正数</summary>
    public int MisfireThresholdSeconds { get; set; } = 60;

    /// <summary>在飞执行上限,默认 8(线程池保护的兜底;达到后本拍不再领取,下拍再来);必须为正数</summary>
...
public class AdminJobsHttpOptions
{
    ...
    /// <summary>
    /// 目标 IP 黑名单(CIDR)。默认封云元数据段的 IPv4/IPv6 两种形态<b>与回环</b>,<b>不封内网</b>——调度器打内网服务是主用途
    /// (RFC1918 与 ULA <c>fc00::/7</c> 照旧放行)。<c>fe80::/10</c> 是 <c>169.254.0.0/16</c> 的 IPv6 孪生,一并默认封。
    /// 回环默认封是因为"任务打宿主自己的管理端口"几乎不是用例,却是一条绕过网络边界的现成通道;真要打本机自行放开。
