...
/// <summary>
/// 缓存逻辑键(禁硬编码字符串纪律)——所有缓存键在此集中定义,不散落魔法串。
/// <para>这里给的是<b>不含前缀</b>的逻辑键;<c>Cache:KeyPrefix</c>(默认 <c>smart:</c>)由
/// <see cref="ICacheProvider"/> 实现统一追加。故实际 Redis 键形如 <c>smart:perm:123</c>。</para>
/// </summary>
public static class CacheKeys
{
    ...
    /// <summary>会话活跃状态(热路径:每受保护请求校验;登出/强退时移除)</summary>
    ...
    /// <summary>
    /// 登录页站点信息(标题/副标题/版权/Logo + 验证码与短信登录开关)整体一份。
    /// <para>它是<b>匿名</b>端点,每次打开登录页都要读;拆成七个键就是七次串行缓存往返,
    /// 装了 Redis 之后那是七个 RTT。任一相关配置改动即整体失效。</para>
    /// </summary>
    public const string SiteInfo = "config:site";

    /// <summary>某账号的连续登录失败次数(登录锁定用;成功登录即清除,窗口过期自动归零)</summary>
    public static string LoginFail(string account) => $"loginfail:{account}";

    /// <summary>某验证码票据对应的明文(登录时一次性校验并消费,短 TTL 过期)</summary>
    public static string Captcha(string captchaId) => $"captcha:{captchaId}";

    /// <summary>某短信验证码明文。<paramref name="purpose"/>=<c>mfa</c> 时 key 为挑战 Id(码绑定已过密码的用户)、<c>login</c> 时为规范化手机号;一次性消费,TTL 过期。</summary>
    ...
    /// <summary>TOTP 登录挑战 → 已过密码校验的 userId(一次性消费;TTL 见 Totp 选项)</summary>
    ...
    /// <summary>外部登录授权态(<c>state</c>)→ {providerCode, nonce, codeVerifier, redirectUri}。防 CSRF 兼承载 PKCE/nonce;回调时按 <c>state</c> 一次性消费,短 TTL 过期。</summary>
    public static string OAuthState(string state) => $"oauth:state:{state}";

    /// <summary>外部登录成功后令牌对的一次性交付票据 → LoginOutput。令牌<b>不进</b>重定向 URL,前端凭票据换取;一次性消费(<see cref="ICacheProvider.GetAndRemoveAsync{T}"/>),短 TTL 过期。</summary>
    public static string OAuthTicket(string ticket) => $"oauth:ticket:{ticket}";

    /// <summary>
    /// 未绑定外部登录的「待绑定」票据 → 已解析的外部身份字段(现场绑定流程用)。
    ...
    /// <summary>
    /// 固定窗口限流计数。<paramref name="bucket"/> 为 <c>auth</c>(认证端点,更严)或 <c>all</c>;
    /// <paramref name="partition"/> 是分区键:用户端按客户端 IP,机器端(带 API Key 头)按 key 的哈希——两边不同桶、各自计数;
    ...
    /// </summary>
    public static string RateLimit(string bucket, string partition, long windowIndex) =>
        $"rl:{bucket}:{partition}:{windowIndex}";
    ...
    /// </summary>
    [Obsolete("节流时刻已并进 SessionCacheInfo.LastPersistedAt(每请求少一次读、每个窗口少一次写);本键不再写入,下个大版本删除。")]
