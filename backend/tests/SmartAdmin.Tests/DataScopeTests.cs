...
/// <summary>
/// 多机构数据范围——直压 SqlSugar 全局过滤器:切换生效范围,同一列表得不同数据集。
/// 数据层测最严谨(等价"两机构用户查同一接口得不同数据")。
/// </summary>
public class DataScopeTests
{
    ...
    {
        // 经仓储插入 DataEntity 时,CreateOrgId 由审计 AOP 从 ICurrentUser.OrgId 回填(否则机构数据范围恒 0 行)
    ...
    {
        // 全局范围过滤器只管查询;仓储写路径对 IOrgScoped 另加范围守卫,堵按主键越权改删他机构行的 IDOR。
