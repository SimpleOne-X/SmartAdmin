...
public class SeedUpgradeTests
{
    ...
    private static Task RestartWithAsync(
        Func<ISqlSugarClient, Task> tamper,
        Func<ISqlSugarClient, Task> assert) =>
        RestartWithAsync(tamper, sp => assert(sp.GetRequiredService<ISqlSugarClient>()));

    /// <summary>同上,断言侧拿到二次启动的整个服务作用域(要经服务层验结果时用)。</summary>
    private static async Task RestartWithAsync(
        Func<ISqlSugarClient, Task> tamper,
        Func<IServiceProvider, Task> assert)
    {
        var dbPath = Path.Combine(Path.GetTempPath(), $"smart-seedup-{Guid.NewGuid():N}.db");

        try
        {
            // 1) 首启:建表 + 播种,得到一个"正常的库"(前提就是从零播种,不能拿模板副本)
            ...
            await assert(s2.ServiceProvider);
        }
        finally
        {
    ...
    /// <summary>
    /// 升级时:结构性种子(菜单)已有行的<b>结构列</b>被刷回种子值(挂载点、路由、图标),版本号落到 Current;
    /// 用户在菜单管理页改过的标题、排序、可见、启用<b>留着</b>——那几列归用户,不在 DefaultMenuSeed.SyncColumns 里。
    /// </summary>
    [Fact]
    public async Task Upgrade_syncs_structural_columns_but_keeps_user_edits()
    {
        await RestartWithAsync(
            async db =>
            {
                await DowngradeVersionAsync(db);
                // 模拟"老库里的旧结构"(挂载点 / 路由 / 图标都跟当前种子不一致)+ 用户在界面上改过的四列
                await db.Updateable<SysMenu>()
                    .SetColumns(x => new SysMenu
                    {
                        ParentId = 999, Path = "/old", Icon = "ph:old",
                        Title = "我的工作台", Sort = 42, Visible = false, Enabled = false,
                    })
            ...
            async db =>
            {
                ...
                Assert.Equal(0, menu.ParentId);                    // 挂载点刷回来了——这正是老库拿不到的那类改动
                Assert.Equal("/workbench", menu.Path);              // 路由刷回
                Assert.Equal("ph:squares-four-duotone", menu.Icon); // 图标刷回
                Assert.Equal("我的工作台", menu.Title);              // 用户改的标题留着
                Assert.Equal(42, menu.Sort);                        // 排序留着
                Assert.False(menu.Visible);                         // 可见留着
                Assert.False(menu.Enabled);                         // 启用留着
    ...
    public async Task Upgrade_never_reverts_user_edited_rows()
    {
        await RestartWithAsync(
            ...
            async db =>
            {
                ...
                Assert.Equal("我司后台", config.ConfigValue);   // 不是 "SmartAdmin"
    ...
    /// <para>场景:管理员在角色页把某默认用户的角色删了又重授,重授走 AOP 拿到<b>雪花号</b>,
