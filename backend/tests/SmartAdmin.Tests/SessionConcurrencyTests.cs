...
/// <summary>
/// 会话并发上限在<b>没有进程内锁</b>时依然成立。
///
/// <para><b>为什么"先腾位、再插入"这种写法必须靠锁</b>:先读活跃集合 → 算该踢几个 → 踢 → 再插入,是个
/// 读-改-写,并发下两个登录各自读到<b>同一份</b>旧集合、各自都算出"不用踢",于是双双插入 → 上限被突破。
/// 靠一个进程内的锁把它串行化,只在单进程里有效:两个副本上的并发登录照样各读各的,
/// <c>SessionMode.Single</c>(挤下线)踢不掉旧会话、<c>MaxConcurrent</c> 被突破。换 Redis 也修不好——这不是缓存问题。</para>
///
/// <para><b>本类验证的写法:先插入、再收敛</b>。插完再读,两个并发登录<b>都能看见对方的新行</b>,
/// 于是都算出同一个"只保留最新 N 个"的答案 → 收敛到上限。吊销是幂等的(只写 <c>RevokedAt IS NULL</c> 的行),
/// 重复吊销无害。不需要任何锁,也就不需要分布式锁。</para>
///
/// <para>注意:一个 <c>static</c> 的进程内锁会被同进程内的两个 host <b>共用</b>——所以"起两个 host"并不能
/// 在测试里复现"先腾位、再插入"的并发 bug。真正能证伪的实验是<b>去掉锁</b>:去掉后 enforce-before-insert 立刻溢出上限,
...
public class SessionConcurrencyTests
{
    ...
    private static AdminAppFactory Factory(string mode, int maxConcurrent) => new()
    {
        ...
        {
            ["SmartAdmin:Security:Session:Mode"] = mode,
            ["SmartAdmin:Security:Session:MaxConcurrent"] = maxConcurrent.ToString(),
        },
    };
    ...
    private static async Task<List<SysSession>> ActiveSessions(AdminAppFactory f)
    {
        ...
        // (见 TestTenantContext.cs)。这些会话都是真实登录写入、已经带着正确 TenantId,这里只是验证态验证读。
        return await sessions.AsQueryable().ClearFilter<ITenantScoped>()
