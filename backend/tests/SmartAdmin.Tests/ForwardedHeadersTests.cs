...
/// <summary>
/// 反向代理后取真实客户端 IP。限流分区、登录日志、爆破防护全挂在"客户端 IP"这一个值上——
...
public class ForwardedHeadersTests
{
    ...
    {
        var settings = new Dictionary<string, string?> { ["SmartAdmin:Api:ForwardedHeaders:Enabled"] = "true" };
        for (var i = 0; i < knownProxies.Length; i++)
            settings[$"SmartAdmin:Api:ForwardedHeaders:KnownProxies:{i}"] = knownProxies[i];
