...
public class DiagnosticsTests
{
    ...
    /// <summary>
    /// <see cref="ScalarAccessAuthorizationHandler"/> 是 Minimal API 授权中间件的 Policy 判定,与
    /// <see cref="RolePermissionAttribute"/> 走的是两条不通用的管线,但共用同一份 <c>smartadmin.authorize</c>
    /// 埋点——这里验的是 Minimal API 这条路径没有漏埋。生产环境 + Scalar 网关开启才会真正跑到这个 handler
    /// (见 <c>ScalarDocsTests</c>),超管走的是免查库的放行分支,与两个既有用例断言的 allow 场景对齐。
    /// </summary>
    [Fact]
    public async Task ScalarAccess网关放行会开span并带上allow结果()
    {
        var spans = new List<Activity>();
        using var listener = new ActivityListener
        {
        ...
        await c.GetAsync("/openapi/v1.json");
    ...
    /// <summary>同上,验证 <c>smartadmin.authorizations</c> 计数器也覆盖了 Minimal API 这条路径,
    /// 与 <see cref="RolePermissionAttribute"/> 的 MVC 路径共用同一个仪表。</summary>
    [Fact]
    public async Task ScalarAccess网关放行时授权计数器打上allow标签()
    {
        var counted = new List<(string Instrument, long Value, string? Result)>();
        using var meterListener = new MeterListener();
        meterListener.InstrumentPublished = (instrument, l) =>
