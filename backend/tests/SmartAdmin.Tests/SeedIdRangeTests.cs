...
/// <summary>
/// 种子主键保留区间的契约测试(见 <see cref="SmartSeedIds"/>)。
/// <para>守几件事:内核自己不得越出 <c>[1, KernelMax]</c>(否则会吃掉消费者的号段,发包后无法回收);
/// 消费者种子不得低于 <c>ConsumerMin</c>;以及运行时上限——启动时刻动态算出的雪花地板
/// (<see cref="SnowflakeIdGenerator.CurrentFloor"/>)——既拦得住真正危险的越界 Id,又不必像固定区间那样
/// 把消费者限制在 3096 个连续整数编号内。</para>
/// </summary>
public class SeedIdRangeTests
{
    ...
    [
        typeof(ISeedData).Assembly,    // SmartAdmin.SqlSugar —— SchemaVersionSeed 藏在这里,最容易漏
        typeof(RbacService).Assembly,  // SmartAdmin.Services —— 其余 6 个内置种子
    ];
    ...
    public void BuiltInSeeds_AllIdsWithinKernelRange()
    {
        ...
        foreach (var seed in scope.ServiceProvider.GetServices<ISeedData>())
        {
            ...
            {
                Assert.True(id is >= 1 and <= SmartSeedIds.KernelMax,
                    $"内置种子 {seed.GetType().Name} 的 Id {id} 越出内核保留区间 [1, {SmartSeedIds.KernelMax}]。" +
                    $"内核只能用低号段:{SmartSeedIds.ConsumerMin} 起是留给消费者的,占了它,消费者升级内核包时会主键冲突。");
                checkedRows++;
            }
        }
    ...
    /// 上限的校验见 <see cref="ConsumerSeed_LargeSemanticIdIsAccepted"/> 与
    /// <see cref="ConsumerSeed_IdAtOrAboveLiveFloorFailsAtStartup"/>,本用例不覆盖。</summary>
    [Fact]
    public void ConsumerSeeds_AllIdsAtOrAboveConsumerMin()
    {
        ...
        foreach (var seed in scope.ServiceProvider.GetServices<ISeedData>())
        {
            ...
            foreach (var id in SeedIds(seed))
                Assert.True(id >= SmartSeedIds.ConsumerMin,
                    $"消费者种子 {seed.GetType().Name} 的 Id {id} 低于消费者下界 {SmartSeedIds.ConsumerMin}。");
        }
    }

    /// <summary>
    /// 消费者种子可以用远超 4095 的语义化编号(如按业务模块拼的号段):一段写死的 <c>[1000, 4095]</c> 区间
    /// 只有 3096 个连续整数槽位,菜单多的系统很难编出有意义的号。上限是启动时刻动态算出的雪花地板,
    /// 只要不撞见 <see cref="ConsumerSeed_IdAtOrAboveLiveFloorFailsAtStartup"/> 那种量级,启动应正常通过。
    /// </summary>
    [Fact]
    ...
    /// 这是动态雪花地板检查真正咬合的那一层,漏了它,过大的种子迟早会被自己实例发出的号追上,静默撞主键。
    ...
    private static IEnumerable<long> SeedIds(ISeedData seed)
    {
        ...
        // 按 PrimaryId 取号:种子契约已放宽到它,明细表(无审计列)同样可种子化
        return rows.Cast<PrimaryId>().Select(r => r.Id);
    }

    /// <summary>业务语义编号范例:产品线 9020 + 序号 010,这类远大于 4095 的号段应被接受(上限是动态雪花地板,不是固定字面量)。</summary>
