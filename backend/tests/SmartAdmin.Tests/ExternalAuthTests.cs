...
/// <summary>
/// 外部登录 / SSO 行为锁。用一个假 <see cref="IExternalAuthProvider"/> 顶替真 IdP(不依赖网络),
/// 覆核心分支:未绑定默认拒绝、开启自动开户则建号+绑定、已绑定则复用会话、未知 provider 拒、绑定唯一。
/// </summary>
public class ExternalAuthTests
{
    ...
    // 记下控制器在授权阶段交给 provider 的入参
    private sealed class AuthorizeCapturingProvider : IExternalAuthProvider
    {
        public ExternalAuthorizeRequest? Last { get; private set; }
        public string Code => "capture";
        public string DisplayName => "Capture";
        public string? Icon => null;
        public Task<string> BuildAuthorizeUrlAsync(ExternalAuthorizeRequest request, CancellationToken cancellationToken = default)
        {
            Last = request;
            return Task.FromResult("https://idp.test/authorize");
        }
        public Task<ExternalIdentity> ExchangeAsync(ExternalExchangeRequest request, CancellationToken cancellationToken = default)
            => Task.FromResult(new ExternalIdentity("capture", "sub-capture"));
    }
    ...
    public async Task Unbound_external_login_is_rejected_by_default()
    {
        ...
        Assert.Equal(ErrorCode.OAuthAccountNotBound, ex.Code);   // 安全默认:未绑定 = 拒绝
    }
    ...
    private static Task EnableLinkByAccountAsync(IServiceProvider sp, string provider) =>
        sp.GetRequiredService<IConfigService>().AddAsync(new ConfigInput
        {
            ConfigKey = $"sys.externalauth.{provider}.linkByAccount",
            ConfigValue = "true",
            Name = $"外部登录-{provider}-按账号自动关联",
            GroupCode = "externalauth",
        });

    [Fact]
    public async Task Link_by_account_is_off_by_default()
    {
        var identity = new ExternalIdentity("test", "same-name-off");
        using var f = Factory(identity);
        using var scope = f.Services.CreateScope();
        var sp = scope.ServiceProvider;
        await InsertUserAsync(sp, "same-name-off");   // 同名本地账号在,但开关没开

        var ex = await Assert.ThrowsAsync<AdminException>(() => sp.GetRequiredService<IAuthService>().LoginByExternalAsync(Input("test")));
        Assert.Equal(ErrorCode.OAuthAccountNotBound, ex.Code);
        Assert.Null(await sp.GetRequiredService<ISysUserExternalService>().FindByExternalAsync("test", "same-name-off"));
    }

    [Fact]
    public async Task Link_by_account_binds_the_same_named_account_on_first_login()
    {
        var identity = new ExternalIdentity("test", "zhangsan-link");
        using var f = Factory(identity);
        using var scope = f.Services.CreateScope();
        var sp = scope.ServiceProvider;
        var auth = sp.GetRequiredService<IAuthService>();
        var user = await InsertUserAsync(sp, "zhangsan-link");
        await EnableLinkByAccountAsync(sp, "test");

        Assert.Equal(user.Id, (await auth.LoginByExternalAsync(Input("test"))).UserId);
        var binding = await sp.GetRequiredService<ISysUserExternalService>().FindByExternalAsync("test", "zhangsan-link");
        Assert.Equal(user.Id, binding!.UserId);

        // 关联只发生在首登;之后以绑定为准,开关关掉照样能登
        await sp.GetRequiredService<IConfigService>().SaveValuesAsync(
            [new ConfigBatchItem { ConfigKey = "sys.externalauth.test.linkByAccount", ConfigValue = "false" }]);
        Assert.Equal(user.Id, (await auth.LoginByExternalAsync(Input("test"))).UserId);
    }

    [Fact]
    public async Task Link_by_account_never_takes_super_admin_disabled_or_already_bound_accounts()
    {
        using var f = Factory(new ExternalIdentity("test", "unused"));
        using var scope = f.Services.CreateScope();
        var sp = scope.ServiceProvider;
        var auth = sp.GetRequiredService<IAuthService>();
        ...
        var disabled = await InsertUserAsync(sp, "link-disabled");
        disabled.Enabled = false;
        await sp.GetRequiredService<IRepository<SysUser>>().UpdateAsync(disabled);

        var bound = await InsertUserAsync(sp, "link-bound");
        await ext.BindAsync(bound.Id, new ExternalIdentity("test", "the-real-owner"));

        // 超管、停用账号、已绑过本 provider 的账号、没有同名账号:都不关联,交回未绑定策略(默认拒绝)
        foreach (var subject in new[] { "superAdmin", "link-disabled", "link-bound", "no-such-account" })
        {
            var ex = await Assert.ThrowsAsync<AdminException>(() =>
                auth.LoginByExternalIdentityAsync(new ExternalIdentity("test", subject)));
            Assert.Equal(ErrorCode.OAuthAccountNotBound, ex.Code);
            Assert.Null(await ext.FindByExternalAsync("test", subject));
        }
        Assert.Equal("the-real-owner", Assert.Single(await ext.ListByUserAsync(bound.Id)).Subject);
    }
    ...
    [Fact]
    public async Task Admin_provider_list_reports_the_link_by_account_switch()
    {
        using var f = Factory(new ExternalIdentity("test", "sub-link-flag"));
        var admin = f.CreateClient();
        admin.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue(
            "Bearer", await admin.LoginToken("superAdmin", "Test@123456"));

        async Task<bool> LinkByAccountAsync()
        {
            var env = await (await admin.GetAsync("/api/v1/auth/external/providers/all")).ReadEnvelope();
            return Assert.Single(env.GetProperty("data").EnumerateArray(), p => p.GetProperty("code").GetString() == "test")
                .GetProperty("linkByAccount").GetBoolean();
        }

        Assert.False(await LinkByAccountAsync());   // 缺省关

        // 配置中心卡片上的开关走批量存值;键不存在时按 externalauth 分组自动落库
        var save = await admin.PutJson("/api/v1/sys/config/batch", new object[]
        {
            new { configKey = "sys.externalauth.test.linkByAccount", configValue = "true" },
        });
        Assert.Equal(0, (await save.ReadEnvelope()).GetProperty("code").GetInt32());
        Assert.True(await LinkByAccountAsync());
    }
    ...
    [Fact]
    public async Task Authorize_hands_the_browser_user_agent_and_an_alphanumeric_state_to_the_provider()
    {
        ...
        const string UA = "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) Mobile/14F89 wxwork/4.1.20 MicroMessenger/7.0.1";
        var req = new HttpRequestMessage(HttpMethod.Get, "/api/v1/auth/external/capture/authorize");
        req.Headers.TryAddWithoutValidation("User-Agent", UA);
        var resp = await client.SendAsync(req);

        Assert.Equal(HttpStatusCode.Redirect, resp.StatusCode);
        Assert.Equal(UA, provider.Last!.UserAgent);
        // 企业微信 / 微信的 OAuth 只收 a-zA-Z0-9、不超过 128 字节的 state
        Assert.Matches("^[A-Za-z0-9]{1,128}$", provider.Last.State);
    }
    ...
    [InlineData("https://gw.example.com/admin")]   // 网关子路径部署
    public void Callback_base_url_accepts_backend_root_addresses(string value) =>
        new ServiceCollection().AddExternalAuthProviders(new AdminExternalAuthOptions { CallbackBaseUrl = value });

    [Theory]
    [InlineData("https://admin.example.com:8443/oauth/callback")]                  // 填成了前端结果页
    [InlineData("https://admin.example.com/api/v1/auth/external/wecom/callback")]  // 填成了整条回调地址
    [InlineData("https://admin.example.com/?from=wecom")]
    [InlineData("https://admin.example.com/#/login")]
    [InlineData("admin.example.com")]
    [InlineData("ftp://admin.example.com")]
    public void Callback_base_url_rejects_values_that_would_build_a_broken_redirect_uri(string value)
    {
        var ex = Assert.Throws<InvalidOperationException>(() =>
            new ServiceCollection().AddExternalAuthProviders(new AdminExternalAuthOptions { CallbackBaseUrl = value }));
        Assert.Contains("CallbackBaseUrl", ex.Message);
    ...
    /// <summary>测试宿主:TOTP 能力打开(与 MfaEnrollmentTests 同款),强制对象由用户行的 ForceTotp 决定。</summary>
    private static AdminAppFactory ForcedTotpFactory(IExternalAuthProvider provider) => new()
    {
        Settings = new Dictionary<string, string?>
        {
        ...
        Overrides = s => s.AddSingleton(provider),
    };

    /// <summary>强制 MFA:外部登录已绑用户须 TOTP;未绑定 → TotpNotBound,不可直接出票。</summary>
    [Fact]
    public async Task External_login_of_unbound_force_totp_user_is_rejected()
    {
        var identity = new ExternalIdentity("sso-mfa", "sub-force");
        using var f = ForcedTotpFactory(new FakeExternalAuthProvider(identity));
        using var scope = f.Services.CreateScope();
        var sp = scope.ServiceProvider;
    ...
    /// <summary>外部登录已绑且已启用 TOTP → 40018 信令 + challengeId(供回调页完成流)。</summary>
    [Fact]
    public async Task External_login_totp_required_carries_challenge_id()
    {
