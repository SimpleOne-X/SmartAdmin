...
/// <summary>
/// 磁盘回收。锁住上传子系统两处容易只涨不消的地方:软删的文件要删盘、弃单的分片要清掉。
...
public class FileGcTests : IAsyncLifetime
{
    ...
    public FileGcTests()
    {
        _dbFile = Path.Combine(Path.GetTempPath(), $"smart-{_id}.db");
        _root = Path.Combine(Path.GetTempPath(), $"smart-root-{_id}");
    ...
    {
        // 秒传去重:两条独立记录共享同一物理文件(a 落盘,b 只是引用同一 StoragePath)
    ...
    public async Task FileGc_TraversalPath_DoesNotTouchOutsideRoot_AndKeepsSweeping()
    {
        ...
        var outside = Path.Combine(Path.GetTempPath(), $"smart-outside-{_id}.txt");
    ...
    public async Task ChunkComplete_DiscardsParts_OnSuccessAndOnPermanentRejection()
    {
        ...
        Assert.False(Directory.Exists(partsDir));   // ← 失败路径不清理的话,分片会一直留在盘上
