...
/// <summary>
/// 执行器:重试共享 FireInstanceId、超时/取消不重试、47005 失败行、
...
public class JobExecutorTests : IAsyncLifetime
{
    ...
    public JobExecutorTests()
    {
        ...
        {
            services.AddSingleton<IEmailSender>(_email);   // 前置注册压过内核默认(同 ReplaceabilityTests 的姿势)
    ...
    public async Task Local_kill_cancels_run_without_counting_failure()
    {
        ...
        Assert.True(await _host.CancelLocalAsync(running.Id));
        await fire;
    ...
    public async Task TryFire_serial_skip_rejects_concurrent_manual_fires_atomically()
    {
        ...
        var logs = await _host.WaitForLogsAsync(job.Id, l => l.Count == 1);
        Assert.True(await _host.CancelLocalAsync(logs[0].Id));
        await started;
    }
    ...
    public async Task TryFire_never_exceeds_max_concurrent_runs()
    {
        ...
        try
        {
            ...
            foreach (var log in open)
                await _host.CancelLocalAsync(log.Id);
