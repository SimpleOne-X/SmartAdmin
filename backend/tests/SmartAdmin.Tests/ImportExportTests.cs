...
/// <summary>
/// 导入导出领域/HTTP 测试。每条上方注释写清「变异什么会让它红」。
...
public class ImportExportTests
{
    ...
    private static ImportRow Row(int index, string account, string name, string orgCode = "smart_tech",
    ...
    /// <summary>
    /// 导出 xlsx 端点返回文件流,不进 Result&lt;T&gt; 信封。
    ...
    /// <summary>
    /// ExportAsync 不得被 PagedListExtensions.MAX_SIZE=200 截断。
    ...
    public async Task PreviewRows_FedBackTo_ValidateAndCommit_AreIdempotentOnDictColumns()
    {
        ...
        await using var sheet = await writer.WriteAsync(new ExportSheet
        {
            ...
            [
                ...
                {
                    ["Account"] = "rt-dict-1", ["Name"] = "往返一", ["Gender"] = "男", ["OrgCode"] = "smart_tech",
                },
            ],
        });
    ...
    public async Task Import_OrgCode_ResolvesId_Or_RefNotFound()
    {
        ...
        var ok = await runner.CommitAsync(
            [Row(1, "fk-org-ok", "外键机构对", "smart_tech")],
            profile, DuplicateStrategy.Skip);
        Assert.Equal(1, ok.Inserted);
        var saved = await users.AsQueryable().FirstAsync(u => u.Account == "fk-org-ok");
        Assert.Equal(3, saved!.OrgId); // DefaultOrgSeed: smart_tech Id=3
    ...
    public async Task Preview_ExceedsMaxImportRows_Throws()
    {
        using var f = new AdminAppFactory
        {
            ...
            {
                ["SmartAdmin:Excel:MaxImportRows"] = "2",
            },
        ...
        await using var xlsx = await writer.WriteAsync(new ExportSheet
        {
            ...
            Rows =
            [
                new Dictionary<string, object?> { ["Account"] = "lim1", ["Name"] = "A", ["OrgCode"] = "smart_tech" },
                new Dictionary<string, object?> { ["Account"] = "lim2", ["Name"] = "B", ["OrgCode"] = "smart_tech" },
                new Dictionary<string, object?> { ["Account"] = "lim3", ["Name"] = "C", ["OrgCode"] = "smart_tech" },
            ],
        });
    ...
    public async Task Export_ExceedsMaxExportRows_Throws()
    {
        using var f = new AdminAppFactory
        {
            Settings = new Dictionary<string, string?>
            {
                ["SmartAdmin:Excel:MaxExportRows"] = "1",
            },
        };
        using var scope = f.Services.CreateScope();
        var sp = scope.ServiceProvider;
    ...
    public async Task DemoMode_BlocksImportCommit_AllowsExport()
    {
        using var f = new AdminAppFactory
        {
            Settings = new Dictionary<string, string?> { ["SmartAdmin:DemoMode"] = "true" },
            Overrides = UseRealExcelCodecs,
        };
        var client = f.CreateClient();
        client.DefaultRequestHeaders.Authorization =
    ...
    /// (Commit/Validate 都会深拷贝入参,是防篡改设计,所以只能从返回值上看。)
    /// </para>
    ...
    // ── Import with non-existent role code → error ────────────────
    ...
    // ── Import overwrite to out-of-scope user's org → error ───────
    ...
    public async Task Import_Overwrite_OutOfScopeOrg_Rejected()
    {
        using var f = new AdminAppFactory
        {
            Overrides = s =>
            {
                ...
                s.AddSingleton<IDataScopeContext, SmartAdmin.SqlSugar.DataScopeContext>();
            },
        };
        using var scope = f.Services.CreateScope();
        var sp = scope.ServiceProvider;
