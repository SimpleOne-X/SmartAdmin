...
/// <summary>GitHub / WeChat 可选包:假 HttpMessageHandler 覆盖 mapping、缺 subject、厂商错误(不触网)。</summary>
public class GitHubWeChatAuthProviderTests
{
    ...
    [Fact]
    public void AddSmartAdminGitHubAuth_registers_fixed_code_provider()
    {
        var services = new ServiceCollection();
        services.AddLogging();
        // 真实 DI 路径:配置有 ClientId+Secret 时必须能注册并可解析,不能 ArgumentException
        services.AddSmartAdminGitHubAuth(new GitHubAuthOptions
        {
            ClientId = "cid",
            ClientSecret = "csec",
            DisplayName = "GH",
    ...
    [Fact]
    public void AddSmartAdminWeChatAuth_registers_fixed_code_provider()
    {
        var services = new ServiceCollection();
        services.AddLogging();
        services.AddSmartAdminWeChatAuth(new WeChatAuthOptions
        {
            AppId = "wxapp",
            AppSecret = "wxsec",
        });
