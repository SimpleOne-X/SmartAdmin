...
/// <summary>
/// 导入导出 × 数据范围。三个真实账号、不同数据范围、导出同一业务列表得不同行集,
/// 是覆盖数据范围招牌能力最重要的一类测试。
/// <para>
/// 注:sys_user 继承 BaseEntity 不走 IOrgScoped 全局过滤;
...
public class ImportExportScopeTests
{
    ...
    /// <summary>
    /// 数据范围受限账号导入范围外机构名 → ImportOrgOutOfScope,库无新行。
    ...
    /// <summary>
    /// 数据范围招牌能力:三个不同数据范围真实账号,各打同一个**导出端点** → 行数不同且互不可见对方行。
    /// <para>
