...
- **多租户（共享库模式）一期落地。** 新增 `SysTenant` 注册表与固定 Id=1 的受保护默认租户（零配置启动落脚点，兼老库升级回填目标）；`ITenantScoped`/`TenantEntity` 隔离基类与 `IOrgScoped`/`DataEntity` 并列，机构、用户、角色、岗位、角色菜单、角色数据范围、通知、文件、日志、会话等核心表统一迁入，按 `TenantId` 全局过滤且不设逃逸条件（连超管也不例外）；写路径（Update/Delete/HardDelete）与账号唯一性同步补齐越权与查重兜底（账号全平台唯一，机构/角色/职位编码改为租户内唯一）。登录、短信/TOTP 二次验证、外部登录（SSO）、刷新令牌等预认证路径显式豁免过滤器。JWT 新增 `tid`（所属租户）与 `padm`（平台管理员——与超管完全独立的第二道门，唯一能管理 `SysTenant` 注册表本身的身份）两个 claim。后台新增「租户管理」目录（`system/tenant`，平台管理员可授权查看/操作），新增租户即建首个管理员账号；隔离模式以分段控件展示路线图（共享库为一期唯一可选项，独立库置灰标二期开放）。顶栏与个人资料页只读展示当前所属租户名，不提供切换交互（跨租户运营台是否要做、怎么做，留给后续按真实需求评估）。**存量会话在拿到新令牌前看不到任何租户范围内的数据**（旧 access token 不带 `tid`，过滤器判空即拒绝一切租户范围行），刷新令牌换发的新令牌会补上声明后自愈，生产升级建议在此窗口内引导用户重新登录。

### Fixed

- **SQL 执行失败/超时的语句不再漏记慢 SQL 告警。** SqlSugar 的 `OnLogExecuted`（慢 SQL 判定唯一落点）只由成功路径的 `ExecuteAfter` 触发，语句超时或执行失败直接走 `OnError` 分支，判断整个被跳过——线上排查慢查询时最该被看见的那类语句（跑到超时）反而不在慢 SQL 日志里，只剩异常堆栈。`OnError` 里补上同一套阈值判断，复用同一条"慢 SQL[...]"日志格式。（[#9](https://github.com/SmartCode-X/SmartAdmin/issues/9)）
