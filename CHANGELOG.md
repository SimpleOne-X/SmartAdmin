...
- **角色的「授权菜单」界面重做。** 改成右侧悬浮抽屉：每个目录一张卡片，页面下的按钮做成可勾选的胶囊，行内显示页面路由和已选数，鼠标悬停按钮显示权限码。底部固定显示已授权数量和「取消 / 保存」，列表再长也只有中间滚动。原先的表单弹窗形态下，内容一长保存按钮会被挤出屏幕，现在授权菜单不再跟随全局的弹窗 / 抽屉偏好，固定用抽屉。应用不多于 4 个时用分段控件切换，更多则退回下拉；「未分配」下没有目录时不再占一个空分段。前端语言包里 `role.catalog`、`role.menuCol`、`role.buttonsCol` 三个旧列头文案已删除，覆盖过这三个键的应用可以一并去掉。
